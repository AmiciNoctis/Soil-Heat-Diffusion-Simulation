...
# Definindo a função de temperatura de Ruth Brum
def temperatura_ruth(z, t):
    return u0 + (theta0 * np.exp(-np.sqrt(omega / (2 * k)) * z)) * np.sin(omega * t - np.sqrt(omega / (2 * k)) * z)

# Definindo valores de tempo fixo
t_fixed = [30, 90]  # Tempo fixo em dias

# Definindo valores de profundidade
z_values = np.linspace(0, 100, 100)  # Valores de profundidade de 0 a 15 metros
...
for idx, t in enumerate(t_fixed):
    temperaturas = [temperatura_u(t, z) for z in z_values]
    plt.plot(z_values, temperaturas, label=f'Temperatura - Dia {t}', color=cores[idx])

    # Adicionando as curvas da solução de Ruth Brum (2013)
    temperaturas_ruth = [temperatura_ruth(z, t) for z in z_values]
    plt.plot(z_values, temperaturas_ruth, linestyle='--', color=cores[idx])
...
plt.title('Comparação das soluções de temperatura no solo')
plt.legend()
plt.grid(True)
plt.show()
